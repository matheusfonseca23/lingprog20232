...
void exemploSe() {
	...
	if (media >= 6) { //Se
		//o que ocorre quando verdadeira a condição	
		printf("APROVADO DIRETO\n");
	} else {  //Senão
		//o que ocorre quando falsa a condição
		float prova_final = 5;
		media = (media + prova_final)/2;
		if (media >= 5) {
			printf("APROVADO EM RECUPERACAO");
		} else {
			printf("REPROVADO");
		}
	}
...
void exemploCaso() {
	...
	switch (opcao) {
		case 0:
			printf("Voce escolheu a opcao 0");
			break;
		case 1:
...
void questao05() {
	...
	printf("Número: ");
	scanf("%d", &num);
	if (num % 3 == 0 && num % 7 == 0) { //&& E   || OU
		printf("%d é multíplo de 3 e 7", num);
	} else {
		printf("%d NÃO É multíplo de 3 e 7", num);
	}
}
